...
class Reader:
    ...
    def process_dataset(self, *row_parts):
        ...
        if not self.is_evaluating and self.config.RANDOM_CONTEXTS:
            ...
            #요놈
            context_parts = tf.string_split(row_parts[1], delimiter=' ', skip_empty=False)
            
            all_contexts = tf.stack(context_parts[0:])
            ...
            contexts = context_parts[0:(self.config.MAX_CONTEXTS)]  # (max_contexts,)
