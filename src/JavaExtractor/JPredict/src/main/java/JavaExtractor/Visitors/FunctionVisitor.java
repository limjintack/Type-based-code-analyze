...
public class FunctionVisitor extends VoidVisitorAdapter<Object> {
	...
	private List<String> SplitType(String str) {
		int Remark_Start_index, Remark_End_index;

		StringBuffer str_buffer = new StringBuffer(str);
		if(str.contains("/*")) {
			Remark_Start_index = str.indexOf("/*");
			Remark_End_index = str.indexOf("*/");
			str_buffer.delete(Remark_Start_index, Remark_End_index);
			str = str_buffer.toString();
		}
		if(str.contains("//")) {
			Remark_Start_index = str.indexOf("//");
			Remark_End_index = str.indexOf("\n");
		...
		if(str.contains("<")) {
			...
			int s_index = str.indexOf(">");
			Cramp = str.substring(f_index, s_index+1);
			str = str.substring(0, f_index) + str.substring(s_index+1);
		}
		
		String[] arr_var = str.split(",");
		String type_str = arr_var[0];
		String[] arr_str = type_str.split(" ");
		
		
		
		int len = arr_str.length;
		String var = arr_str[len-1];
		String type = arr_str[len-2]+Cramp;
		
		List<String> result = new ArrayList<String>();
		result.add(type);
		result.add(var);
		for (int i = 1 ; i < arr_var.length; i++) {
			type = arr_var[i];
			result.add(type);
		}
		return result;
				...
				for (int j = 0; j < Childs.size(); j++) {
					try {
						if (Childs.get(j) instanceof VariableDeclarationExpr) {
							Split_Type = SplitType(Childs.get(j).toString());
							for (int p = 1 ; p < Split_Type.size(); p++) {
								Variable_List.put(Split_Type.get(p), Split_Type.get(0));
							}
						}
					}
					catch(Exception e1) {
	...
	private void visitVariable(ClassOrInterfaceDeclaration node, Object obj) {
		...
		for (int i = 0; i < c_Type.size(); i++) {
			try {
				Split_Type = SplitType(c_Type.get(i).toString());
				for (int p = 1 ; p < Split_Type.size(); p++) {
					Field_Types.put(Split_Type.get(p), Split_Type.get(0));
				}
				}
			catch(Exception e1) {
				continue;
