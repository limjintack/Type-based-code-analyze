            ...
            for line in file:
                ...
                for c in contexts:
                    ...
                    if len(path) > 2:
                        node = path[1].split('|')
                        if len(node) > 15:
                            preprocess_context.append("")
                        else:
                            preprocess_context.append(c)
                            
                if len(preprocess_context) > max_unfiltered:
                    max_unfiltered = len(preprocess_context)

                sum_total += len(preprocess_context)
                if len(preprocess_context) > max_contexts_to_sample:
                    preprocess_context = np.random.choice(preprocess_context, max_contexts_to_sample, replace=False)

                sum_sampled += len(preprocess_context)

                csv_padding = " " * (max_data_contexts - len(preprocess_context))
                total += 1
                outfile.write(target_name + ' ' + " ".join(preprocess_context) + csv_padding + '\n')
