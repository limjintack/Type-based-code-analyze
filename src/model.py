...
class Model:
    ...
    def evaluate(self, release=False):
        ...
        with open(model_dirname + '/f1.txt', 'a') as output_file:
            output_file.write('After %d epochs: Precision: %.5f, recall: %.5f, F1: %.5f \n' % (self.epochs_trained, precision, recall, f1))
    ...
    def build_training_graph(self, input_tensors):
        ...
        with tf.variable_scope('model'):
            ...
            if self.config.USE_MOMENTUM:
                learning_rate = tf.train.exponential_decay(0.00015, step * self.config.BATCH_SIZE,
